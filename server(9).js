...
const APP_ID_LIVE   = '33l2csjcysJyX0CZB22EL';
const REDIRECT_URI  = 'https://elroi4in1-1.onrender.com/callback';
...
function detectAutoHTFZones(b) {
  ...
  for(const tf of HTF_TFS){
    ...
    // ── VALIDITY CHECK — sl1 must be recent (within last 15 candles of window)
    // If the most recent swing low is too old, skip this TF entirely
    const sl1Recency = recent.length - 1 - troughs[troughs.length-1].idx;
    if(sl1Recency > 15) continue; // swing low is too old for this TF

    const sl1 = troughs[troughs.length-1]; // most recent swing low

    // ── PATTERN 1 & 2 — single swing low, price returning ────────────────
    // Zone: A = sl1.low + closePct×range  |  B = sl1.low - passPct×range
    ...
    const sl2 = troughs[troughs.length-2]; // second most recent swing low

    // ── CLASSIFY SL1 vs SL2 ───────────────────────────────────────────────
    const diff    = sl1.low - sl2.low;   // positive = ascending
    const absDiff = Math.abs(diff);
    const avgLow  = (sl1.low + sl2.low) / 2;
    const isAlmostEqual = absDiff / avgLow < ALMOST_EQUAL_TOL;

    let zoneA, zoneB, structType, nextLevel;

    if(isAlmostEqual){
      // Equal / almost equal — use last SL candle range
      const r = sl1.range > 0 ? sl1.range : absDiff > 0 ? absDiff : sl1.low * 0.002;
      zoneA      = parseFloat((sl1.low + closePct * r).toFixed(2));
      zoneB      = parseFloat((sl1.low - passPct  * r).toFixed(2));
      structType = 'equal';
      nextLevel  = sl1.low;
    } else if(diff > 0){
      // Ascending — lows getting higher
      const baseDiff = diff;
      nextLevel  = parseFloat((sl1.low + baseDiff).toFixed(2));
    ...
    const idFull       = `auto_${structType}_${tf}_${sl1.low.toFixed(4)}_${sl2.low.toFixed(4)}`;
    ...
    if(!existCancel){
      ...
      autoStructures.push({ tf, type:structType,
        sl1:sl1.low, sl2:sl2.low, next:nextLevel,
...
function checkHTFZoneUptrend(b) {
  // Auto detection runs on candle updates (H1/H4/D1) — not needed here every tick
  const activeZones = b.htfZones.filter(z=>!z.cancelled);
  if(!activeZones.length||!b.currentPrice) return;

  const nearZone = isPriceInHTFZone(b, b.currentPrice);

  // ── RESUME CHECK — price broke below zone B ───────────────────────────
  if(!nearZone){
    if(b.htfZonePaused){
      // Price is outside ALL zones — check it broke BELOW (not just outside the top)
      const belowAllZones = activeZones.every(z=>{
        const lo=Math.min(z.a,z.b);
        return b.currentPrice < lo; // price is below zone B — support broke
      });
      if(belowAllZones){
        log(b,'✅ Price broke below HTF zone B — resuming');
  ...
  // ── RESUME: at least 2 of M1,M5,M15 uptrend ─────────────────────────
  if(b.htfZonePaused){
    // Re-analyze all three TFs fresh before checking
    ...
    if(upCount >= 2){
      log(b,`✅ ${upCount}/3 TFs uptrend — HTF zone resolved, resuming`);
      b.htfZonePaused=false;
      b.htfPauseReason='';
      setStatus(b,'running','RUNNING');
...
function handleDerivMessage(b, d) {
  if(d.msg_type==='tick'){
    ...
    if(b.botActive){
      checkHTFZoneUptrend(b); // always check HTF — even during a trade
      if(!b.inTrade) checkEntry(b);
    }
  }
