...
const DATAENGINE_MARKETS = ['frxEURUSD', 'frxGBPUSD', 'frxUSDJPY', 'frxUSDCHF', 'frxAUDUSD']; // top-5 forex pairs
...
const DATAENGINE_WS_URL = `wss://ws.derivws.com/websockets/v3?app_id=${APP_ID_LIVE}`;
...
class LiveTickCollector {
  constructor(db, candleBuilder, wsUrl, logger = console) {
    ...
    this.wsUrl = wsUrl;
    this.logger = logger;
    this.sockets = new Map();   // symbol -> WebSocket
  ...
  _connect(symbol) {
    ...
    ws.on('open', () => {
      this.logger.info && this.logger.info(`[dataengine] live tick collection connected for ${symbol}`);
    ...
    ws.on('close', () => {
      if (!this.sockets.has(symbol)) return; // stopSymbol() already cleaned up
      this.logger.warn && this.logger.warn(`[dataengine] live tick socket closed for ${symbol} — reconnecting in 5s`);
      const t = setTimeout(() => { this.sockets.delete(symbol); this._connect(symbol); }, 5000);
      this.reconnectTimers.set(symbol, t);
    });

    ws.on('error', (e) => {
      this.logger.warn && this.logger.warn(`[dataengine] live tick socket error for ${symbol}: ${e.message}`);
    });
  }
  ...
  stopSymbol(symbol) {
    ...
    const t = this.reconnectTimers.get(symbol);
    if (t) { clearTimeout(t); this.reconnectTimers.delete(symbol); }
  ...
  isRunning(symbol) {
    return this.sockets.has(symbol);
  }

  stopAll() {
    for (const symbol of Array.from(this.sockets.keys())) this.stopSymbol(symbol);
...
app.get('/callback', async (req, res) => {
  ...
  res.send(`<!DOCTYPE html><html><head><title>EL ROI — Connecting to Deriv</title>
  ...
  <body><div class="spin"></div><p>Connecting to Deriv...</p>
  ...
  try {
    ...
    logAll('🔍 Fetching account...');
    broadcast({ type: 'live_login_status', status: 'fetching_account', msg: 'Fetching account...' });
    ...
    if(accountsData.data && accountsData.data.length > 0) {
      ...
      currency  = accountsData.data[0].currency || null;
      logAll(`✅ Account found: ${accountId}`);
    } else {
      ...
      currency  = createData.data[0].currency || 'USD';
      logAll(`✅ Account created: ${accountId}`);
    }

    // Step 5: Get OTP → authenticated WebSocket URL
    logAll('🔑 Getting OTP WebSocket URL...');
    broadcast({ type: 'live_login_status', status: 'getting_otp', msg: 'Getting WebSocket token...' });

    liveAuth = { accessToken, accountId, currency, balance: null, loggedIn: true };

    const wssUrl = await getSharedOtpUrl();
    logAll('✅ OTP WebSocket URL obtained');
    ...
    // Step 6: any bot the user already had waiting to start can connect now
    bots.forEach(b => {
      if(b.userStarted && (!b.derivWs || b.derivWs.readyState !== WebSocket.OPEN)) {
        connectDerivLive(b, wssUrl);
      }
    });
    // Open one socket up front so the first bot to hit "start" has a warm login
    if(!bots.some(b=>b.derivWs && b.derivWs.readyState===WebSocket.OPEN)) {
      // no bot needs it yet — nothing further to do until a bot is started
    }
...
function resolveContractFromHistory(ticks, startIndex, params, maxHoldSeconds) {
  ...
  for (let i = startIndex + 1; i < ticks.length; i++) {
    ...
    if (isMultiplier) {
      const profit = multiplierPnl(price);
      if (params.takeProfit > 0 && profit >= params.takeProfit) return finishMulti(profit, price, i);
      if (params.stopLoss   > 0 && profit <= -params.stopLoss)  return finishMulti(profit, price, i);
    }
...
function btBuildParams(b) {
  ...
  if (isMulti) {
    params.multiplier = b.cfg.multiplier;
    params.takeProfit = b.cfg.takeProfit;
    params.stopLoss   = b.cfg.stopLoss;
  } else if (isVanilla) {
...
function backtestPlaceTrade(b, meta) {
  ...
  const result = resolveContractFromHistory(runner.allTicks, runner.currentTickIndex, params, duration);
  if (!result) {
...
function backtestFinalizeResult(runner, b, profit, contractInfo, insufficientData) {
  ...
  b.tradeLog.unshift({
    ...
    level: typeof contractInfo.level === 'number' ? contractInfo.level.toFixed(2) : null,
  ...
  async run() {
    try {
      ...
      for (let i = 0; i < this.allTicks.length; i++) {
        ...
        if (m1Closed && b.botActive && !b.inTrade && !b.lossCountdownPaused) {
          findLevels(b);
        }
