...
function finalizeResult(b,profit) {
  stopTradeTimer(b,b.currentContractId); // stop timer immediately
  b.inTrade=false; // clear for multiplier
...
function handleDerivMessage(b, d) {
  ...
  if(d.msg_type==='buy'){
    ...
    // Track this contract
    b.activeContracts[cid] = { stake: b.cfg.stake, command: b.cfg.command };
    log(b,`📝 Contract: ${cid} | entry: ${d.buy.start_time} | active: ${Object.keys(b.activeContracts).length}`);
    startTradeTimer(b,cid,b.cfg.durationMins*60);
  ...
  if(d.msg_type==='proposal_open_contract'){
    ...
    // Handle multiplier TP/SL
    const contractInfo = b.activeContracts[cid];
    const isMulti = contractInfo?.command==='CALL_MULT'||contractInfo?.command==='PUT_MULT'
                  ||b.cfg.command==='CALL_MULT'||b.cfg.command==='PUT_MULT';
    if(isMulti){
    ...
    if(con.status==='sold'||con.status==='lost'||con.status==='won'||con.is_expired||con.is_settleable){
      b.currentContractId=cid; // set for finalizeResult
      delete b.activeContracts[cid];
      finalizeResult(b,profit);
    }
  }
  ...
  if(d.msg_type==='sell'){
    if(d.sell){
      ...
      b.currentContractId=cid;
      delete b.activeContracts[cid];
      finalizeResult(b,parseFloat(d.sell.sold_for)-b.cfg.stake);
    }
  }
}
