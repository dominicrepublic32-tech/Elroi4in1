...
function detectAutoHTFZones(b) {
  ...
  for(const tf of HTF_TFS){
    ...
    const sl1 = troughs[troughs.length-1]; // most recent unbroken swing low

    // ── ONE ZONE PER TIMEFRAME — best structure wins ──────────────────────
    // If we have 2 swing lows → use ascending/descending/equal (projects NEXT)
    // If we only have 1 swing low → use pattern 1&2 (sl1 itself is the target)

    if(troughs.length >= 2){
      const sl2 = troughs[troughs.length-2];
      ...
      if(isAlmostEqual){
        // Equal / almost equal — zone around sl1 using its own candle range
        const r = sl1.range > 0 ? sl1.range : sl1.low * 0.002;
        nextLevel  = sl1.low;
        zoneA      = parseFloat((nextLevel + closePct * r).toFixed(2));
        zoneB      = parseFloat((nextLevel - passPct  * r).toFixed(2));
        structType = 'equal';
      } else if(diff > 0){
        // Ascending — NEXT is higher than sl1
        const baseDiff = diff;
        ...
        // Descending — NEXT is lower than sl1
      ...
      // ── ZONE B CHECK — if price already closed below zone B, zone is dead ──
      ...
      // ── PATTERN 1 & 2 — only one swing low, price may return ─────────────
      ...
      // Zone B check — if price already closed below zB_12 this zone is dead
