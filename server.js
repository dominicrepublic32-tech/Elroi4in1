...
function detectAutoHTFZones(b) {
  ...
  const RECENCY = { H1:50, H4:30, D1:20 };
...
function checkHTFZoneUptrend(b) {
  ...
  if(!nearZone){
    if(b.htfZonePaused){
      // Check if price is below zone B of ALL zones it was in
      const stillInAny = activeZones.some(z=>{
        const lo=Math.min(z.a,z.b);
        return b.currentPrice >= lo;
      });
      if(!stillInAny){
...
function handleDerivMessage(b, d) {
  ...
  if(d.msg_type==='candles'){
    ...
    if(['H1','H4','D1'].includes(tf)) detectAutoHTFZones(b);
