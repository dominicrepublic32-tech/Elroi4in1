...
function createBot(id) {
  ...
  return {
    ...
    inTrade:false, activeTrades:[], currentContractId:null,
    entryTargets:[],
...
function startTradeTimer(b, contractId, durationSecs) {
  const trade = {
    contractId, durationSecs,
    remainingSecs: durationSecs,
    level: b.currentActiveLevel,
    struct: b.currentStructType,
    timerId: null,
  };
...
function finalizeResult(b,profit,contractId) {
  b.inTrade=false;
  // Stop timer immediately — win or loss
  if(contractId) stopTradeTimer(b,contractId);
  else if(b.currentContractId) stopTradeTimer(b,b.currentContractId);
...
function handleDerivMessage(b, d) {
  ...
  if(d.msg_type==='proposal_open_contract'){
    ...
    if(con.status==='sold'||con.is_expired||con.is_settleable) finalizeResult(b,profit,con.contract_id||b.currentContractId);
  }
...
function stopBot(b) {
  ...
  if(b.activeTrades) b.activeTrades.forEach(t=>clearInterval(t.timerId));
  b.activeTrades=[];
