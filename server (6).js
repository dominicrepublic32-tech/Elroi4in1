...
function createBot(id) {
  ...
  return {
    ...
    entryTargets:[],
    pendingTrades:[], // FIFO queue: trade context awaiting contract ID
...
function broadcastBotState(b) {
  broadcast({
    ...
    activeContracts:Object.entries(b.activeContracts||{}).map(([cid,info])=>({
      contractId:cid, level:info.level, structType:info.structType,
      command:info.command, market:info.market, stake:info.stake,
    })),
...
function checkEntry(b) {
  ...
  for(const struct of b.activeStructures){
    ...
    // Pass trade context directly — no shared fields, avoids cross-trade contamination
...
function placeTrade(b, meta={}) {
  ...
  const isMulti=b.cfg.command==='CALL_MULT'||b.cfg.command==='PUT_MULT';
  if(isMulti) b.inTrade=true; // multiplier needs single-trade lock for sell
  const duration=b.cfg.durationMins*60;
  // Queue this trade's context — attached to the contract once buy confirms.
  // FIFO queue handles overlapping simultaneous entries correctly.
  if(!b.pendingTrades) b.pendingTrades=[];
  b.pendingTrades.push({
    level: meta.level ?? null,
    structType: meta.structType ?? null,
    command: b.cfg.command,
    stake: b.cfg.stake,
    market: b.cfg.market,
    isMulti,
  ...
  log(b,`📤 Proposal: ${type} ${b.cfg.market} $${b.cfg.stake}${isMulti?` x${b.cfg.multiplier}`:` dur:${duration}s`}`);
...
function finalizeResult(b,profit,contractInfo,cid) {
  stopTradeTimer(b,cid); // stop this contract's timer immediately
  // Only clear inTrade if this WAS the multiplier lock (single-trade type)
  ...
  const struct = contractInfo?.structType;
  ...
  const card={
    ...
    struct, command, market, stake, wr,
    contractId: cid,
  };
  ...
  log(b,`${won?'✅ WIN':'❌ LOSS'} #${b.tradeCount} | ${profit>=0?'+':''}$${profit.toFixed(2)} | WR:${wr}% | contract:${cid}`);
...
function handleDerivMessage(b, d) {
  ...
  if(d.msg_type==='buy'){
    ...
    b.currentContractId = cid; // kept for compatibility, not used for results
    // Pop the oldest pending trade context — FIFO matches order of buy confirmations
    const meta = (b.pendingTrades && b.pendingTrades.length) ? b.pendingTrades.shift() : {
      level:null, structType:null, command:b.cfg.command, stake:b.cfg.stake, market:b.cfg.market,
      isMulti: b.cfg.command==='CALL_MULT'||b.cfg.command==='PUT_MULT'
    };
    // Track this contract with its OWN context — no shared fields
    ...
    log(b,`📝 Contract: ${cid} | entry: ${d.buy.start_time} | active: ${Object.keys(b.activeContracts).length}`);
    // Only run a countdown timer for fixed-duration contracts (not multipliers)
    if(!meta.isMulti) startTradeTimer(b,cid,b.cfg.durationMins*60);
    // Subscribe to real-time updates for ALL contract types immediately
    // This ensures barrier hits and early losses are caught instantly
    setTimeout(()=>{
  ...
  if(d.msg_type==='proposal_open_contract'){
    ...
    // Ignore updates for contracts we're not tracking (e.g. stale subs after reconnect)
    if(!contractInfo) return;
    const isMulti = contractInfo.isMulti;
    if(isMulti){
      if(profit>=b.cfg.takeProfit||profit<=-b.cfg.stopLoss)
        b.derivWs.send(JSON.stringify({sell:cid,price:0}));
    }
    // Finalize when contract is done — barrier hit, expired, or sold
